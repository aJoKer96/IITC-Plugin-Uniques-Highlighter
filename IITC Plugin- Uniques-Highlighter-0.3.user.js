...
function wrapper(plugin_info) {
    ...
    var changelog = [
        {
            version: "0.3",
            chnges: ['Addet configuration menu to adjust infill/ring color, display type (infill or ring) and if displayed Results chall be inverted. This configuration is stores localy.'],
        },{
            version: "0.2",
    ...
    var invertCaptured = true;
    var invertScout = false;
    var invertVisit = false;

    var colorCaptured = "#000000";
    var colorScout = "#000000";
    var colorVisit = "#000000";
    ...
    function highlightUniqueCaptures(data) {
        ...
        if(history.captured == true && invertCaptured == false) {
            highlightPortal(portal, HISTORY_TYPE.CAPTURE);
        } else if(history.captured == false && invertCaptured == true) {
            highlightPortal(portal, HISTORY_TYPE.CAPTURE);
        }
    }
    ...
    function highlightUniqueScout(data) {
        ...
        if(history.scoutControlled == true && invertScout == false) {
            highlightPortal(portal, HISTORY_TYPE.SCOUT);
        } else if(history.scoutControlled == false && invertScout == true) {
            highlightPortal(portal, HISTORY_TYPE.SCOUT);
        }
    }
    ...
    function highlightUniqueVisits(data) {
        ...
        if(history.visited == true && invertVisit == false) {
            highlightPortal(portal, HISTORY_TYPE.VISIT);
        } else if(history.visited == false && invertVisit == true) {
            highlightPortal(portal, HISTORY_TYPE.VISIT);
        }
    }
    ...
    function highlightPortal(portal, type) {
        ...
        if(style == MARKER_TYPE.INFILL) {
            ...
            /*
            if (portal) {
                var range = getPortalRange(portal);
                portalRangeIndicator = (
                    range.range > 0
                    ? L.geodesicCircle(coord, range.range, {fill: false, color: color, weight: 5, interactive: false })
    ...
    window.plugin.uniquesHighlighter.menu = function() {
        var html = '<div>' +
            'Visible portals: <span id="' + self.id + '_count_total"></span><br />' +
            'With history: <span id="' + self.id + '_count_history"></span><br />';
        html +=
            '<input type="checkbox" onclick="' + self.namespace + 'settings.showwhenzoomedout = this.checked; ' + self.namespace + 'onzoomend(); ' + self.namespace + 'storesettings();" id="showwhenzoomedouttoggle">' +
            '<label for="showwhenzoomedouttoggle" style="user-select: none">Show markers when zooming out</label><br />';
        html +=
            '<input type="checkbox" onclick="' + self.namespace + 'settings.invertresults = this.checked; ' + self.namespace + 'invertresults(); ' + self.namespace + 'storesettings(); ' + self.namespace + 'menu();" id="invertresultstoggleZ>' +
    ...
    var setup = function() {
        ...
        $('#toolbox').append('<a onclick="window.plugin.uniquesHighlighter.menu(); return false;" href="#">' + PLUGIN_NAME + '</a>');
